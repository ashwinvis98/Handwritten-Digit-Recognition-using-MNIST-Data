...
train <- read.csv ("mnist _ train.csv" , header= FALSE)
splittingseed <- 100
set.seed(splittingseed)
selsize <-floor(0.7*nrow(train) )
...
# Plotting the graph of C vs Errors
...
#Predicting on the test set using trained SVM model
...
Accracytestfinal <- (sum(correcttestfinal) /nrow(testfinal) )*100
